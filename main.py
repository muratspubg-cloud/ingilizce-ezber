...
CSV_URL_4 = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRWkeuvtou-l6Kf2ne7gVgJQVVBkWeFqjwXxEeoNQJAHkWsszof5hF6rbJBo_n3uET4ojTWlIGCkAPQ/pub?output=csv"
...
class KelimeParcasi(Button):
    ...
    def ciz(self, *args):
        self.canvas.before.clear()
        with self.canvas.before:
            Color(*self.background_color); RoundedRectangle(pos=self.pos, size=self.size, radius=[12])
...
class HarfParcasi(Button):
    ...
    def ciz(self, *args):
        self.canvas.before.clear()
        with self.canvas.before:
            Color(*self.background_color); RoundedRectangle(pos=self.pos, size=self.size, radius=[10])
...
class VeriYoneticisi:
    def __init__(self):
        ...
        self.dosya_yolu_4 = self.yol_bul(self.fiil_dosya)
        ...
        self.fiil_veriler = self.yukle(self.dosya_yolu_4, FIILLER_YEDEK, tip="fiil")
        ...
        try:
            with open(yol, 'w', encoding='utf-8-sig') as f:
                if ad == 'eng_eng.csv':
                    ...
                    for v in ENG_ENG_YEDEK: f.write(f"1;{v['word']};{v['def']};{v['tr']};{v['ex']}\n")
                elif ad == 'fiiller.csv':
                    f.write("Sıra;V1;V2;V3;TR\n")
                    for v in FIILLER_YEDEK: f.write(f"1;{v['v1']};{v['v2']};{v['v3']};{v['tr']}\n")
                else:
                    veri = DIL_KURSU_YEDEK if ad == 'dilkursu.csv' else YEDEK_VERILER
                    f.write("Sıra;TR;EN;IPA;Okunuş;CümleEN;CümleTR\n")
                    for v in veri: f.write(f"1;{v['tr']};{v['en']};{v.get('ipa','')};{v['okunus']};{v.get('cen','')};{v.get('ctr','')}\n")
        except: pass

    def internetten_guncelle(self):
        try:
            for url, yol in [(CSV_URL, self.dosya_yolu_1), (CSV_URL_2, self.dosya_yolu_2), 
                             (CSV_URL_3, self.dosya_yolu_3), (CSV_URL_4, self.dosya_yolu_4)]:
                if "http" in url:
                    r = requests.get(url, timeout=15)
                    r.raise_for_status()
                    with open(yol, 'wb') as f: f.write(r.content)
            
            self.veriler = self.yukle(self.dosya_yolu_1, YEDEK_VERILER, tip="standart")
            self.dil_kursu_veriler = self.yukle(self.dosya_yolu_2, DIL_KURSU_YEDEK, tip="standart")
            self.eng_eng_veriler = self.yukle(self.dosya_yolu_3, ENG_ENG_YEDEK, tip="eng_eng")
            self.fiil_veriler = self.yukle(self.dosya_yolu_4, FIILLER_YEDEK, tip="fiil")
            return True, "Tüm Listeler (Fiiller Dahil) Güncellendi!"
                ...
                with open(yol, 'r', encoding='utf-8-sig') as f:
                    ...
                    if content:
                        ...
                        for i in range(start, len(rows)):
                            ...
                            if tip == "standart":
                                liste.append({"tr": safe(1), "en": safe(2), "ipa": safe(3), "okunus": safe(4), "cen": safe(5), "ctr": safe(6)})
                            elif tip == "eng_eng":
                                liste.append({"word": safe(1), "def": safe(2), "tr": safe(3), "ex": safe(4)})
                            elif tip == "fiil":
                                liste.append({"v1": safe(1), "v2": safe(2), "v3": safe(3), "tr": safe(4)})
            except: pass
...
class AyarlarEkrani(Screen):
    ...
    def gorsel_guncelle(self, hiz):
        ...
        renk(self.btn_yavas, hiz == 0.75); renk(self.btn_normal, hiz == 1.0); renk(self.btn_hizli, hiz == 1.25)
...
class AnaMenu(Screen):
    ...
    def on_enter(self): self.ana_menuyu_ciz()
    ...
    def temizle_ve_ciz(self): self.grid_menu.clear_widgets()
    ...
    def ana_menuyu_ciz(self):
        ...
        self.grid_menu.add_widget(btn_kelime); self.grid_menu.add_widget(btn_cumle); self.grid_menu.add_widget(btn_fiil)
        self.grid_menu.add_widget(alt_grid); self.grid_menu.add_widget(btn_guncelle); self.grid_menu.add_widget(btn_cikis)
    ...
    def kelime_alt_menu_ciz(self, instance):
        ...
        self.grid_menu.add_widget(b1); self.grid_menu.add_widget(b2); self.grid_menu.add_widget(b3); self.grid_menu.add_widget(b4); self.grid_menu.add_widget(b_geri)
    ...
    def cumle_alt_menu_ciz(self, instance):
        ...
        b_geri = OzelButon(text="< Geri Dön", background_color=(0.4, 0.4, 0.4, 1), size_hint=(1, None), height=self.HEDEF_YUKSEKLIK)
        b_geri.bind(on_press=lambda x: self.ana_menuyu_ciz())
        self.grid_menu.add_widget(b1); self.grid_menu.add_widget(b2); self.grid_menu.add_widget(b3); self.grid_menu.add_widget(b_geri)
    ...
    def dil_kursu_baslat_kelime(self, instance): self.baslat_calisma("kelime", YONETICI.dil_kursu_veriler)
    def dil_kursu_baslat_cumle(self, instance): self.baslat_calisma("cumle", YONETICI.dil_kursu_veriler)
    def baslat_calisma(self, mod, liste):
        if not liste:
            Popup(title='Uyarı', content=Label(text='Liste Boş!'), size_hint=(0.8, 0.4)).open(); return
...
class LevelEkrani(Screen):
    def __init__(self, **kwargs):
        ...
        self.spinner = Spinner(text='Level Seçiniz', values=(), size_hint=(1, 0.15), pos_hint={'center_x': .5, 'center_y': .5}, background_color=(0.2, 0.6, 0.8, 1), font_size='24sp', option_cls=BuyukSpinnerOption, dropdown_cls=KaydirilabilirDropDown)
    ...
    def on_pre_enter(self):
        if self.hedef_mod == "eng_eng": aktif_liste = YONETICI.eng_eng_veriler
        else: aktif_liste = YONETICI.veriler
    ...
    def level_secildi(self, spinner, text):
        ...
        try:
            ...
            if self.hedef_mod == "eng_eng": secili_liste = YONETICI.eng_eng_veriler[baslangic:bitis]
            else: secili_liste = YONETICI.veriler[baslangic:bitis]
            if not secili_liste: Popup(title='Hata', content=Label(text='Bu Level Boş!'), size_hint=(0.6, 0.3)).open(); return
            if self.hedef_mod in ["kelime", "cumle", "eng_eng"]:
                ekran = self.manager.get_screen('calisma'); ekran.baslat_ozel(self.hedef_mod, secili_liste); self.manager.current = 'calisma'
            elif self.hedef_mod == "etkinlik_cumle":
                ekran = self.manager.get_screen('etkinlik_cumle'); ekran.baslat(secili_liste); self.manager.current = 'etkinlik_cumle'
            elif self.hedef_mod == "etkinlik_kelime":
                ekran = self.manager.get_screen('etkinlik_kelime'); ekran.baslat(secili_liste); self.manager.current = 'etkinlik_kelime'
...
class KelimeEtkinlikEkrani(Screen):
    ...
    def yeni_soru(self):
        ...
        self.lbl_ipucu.text = f"[b]{self.aktif_veri['tr']}[/b]"; self.lbl_ipucu.markup = True
        kelime = self.aktif_veri['en']; kelime_temiz = re.sub(r"\(.*?\)", "", kelime); temiz_kelime = re.sub(r'[^a-zA-Z]', '', kelime_temiz).lower()
        self.dogru_siralama = list(temiz_kelime)
        karisik_harfler = self.dogru_siralama.copy(); random.shuffle(karisik_harfler)
        for harf in karisik_harfler: btn = HarfParcasi(harf=harf.upper()); btn.bind(on_press=self.harf_tasima); self.harf_havuzu.add_widget(btn)
    def harf_tasima(self, btn):
        if btn.parent == self.harf_havuzu:
            self.harf_havuzu.remove_widget(btn); self.cevap_kutusu.add_widget(btn); self.kullanici_siralama.append(btn.text.lower())
        else:
    ...
    def kontrol_et(self, instance):
        if self.kullanici_siralama == self.dogru_siralama: Popup(title='Tebrikler!', content=Label(text='✅ Doğru Yazdınız!', font_size='24sp'), size_hint=(0.6, 0.3)).open(); SES.oku("Correct!")
        else: Popup(title='Hata', content=Label(text='❌ Yanlış Harfler\nTekrar Dene', font_size='20sp', halign='center'), size_hint=(0.6, 0.3)).open()
...
class EtkinlikEkrani(Screen):
    ...
    def baslat(self, liste=None):
        if liste: self.calisma_listesi = liste
        elif not self.calisma_listesi and YONETICI.veriler: self.calisma_listesi = YONETICI.veriler
        self.yeni_soru()
    def yeni_soru(self):
        self.cevap_kutusu.clear_widgets(); self.kelime_havuzu.clear_widgets(); self.kullanici_siralama = []
        if not self.calisma_listesi: return
        self.aktif_veri = random.choice([v for v in self.calisma_listesi if v.get('cen')])
        self.lbl_ipucu.text = f"[b]{self.aktif_veri['ctr']}[/b]"; self.lbl_ipucu.markup = True
        cumle = self.aktif_veri['cen']; temiz_cumle = re.sub(r'[^\w\s]', '', cumle); self.dogru_siralama = temiz_cumle.split()
        karisik_kelimeler = self.dogru_siralama.copy(); random.shuffle(karisik_kelimeler)
        for kelime in karisik_kelimeler: btn = KelimeParcasi(metin=kelime); btn.bind(on_press=self.kelime_tasima); self.kelime_havuzu.add_widget(btn)
    def kelime_tasima(self, btn):
        if btn.parent == self.kelime_havuzu:
            self.kelime_havuzu.remove_widget(btn); self.cevap_kutusu.add_widget(btn); self.kullanici_siralama.append(btn.text)
        else:
            self.cevap_kutusu.remove_widget(btn); self.kelime_havuzu.add_widget(btn)
            if btn.text in self.kullanici_siralama: self.kullanici_siralama.remove(btn.text)
    def kontrol_et(self, instance):
        if self.kullanici_siralama == self.dogru_siralama: Popup(title='Tebrikler!', content=Label(text='✅ Doğru Cevap!', font_size='24sp'), size_hint=(0.6, 0.3)).open(); SES.oku("Correct!")
        else: Popup(title='Hata', content=Label(text='❌ Yanlış Sıralama\nTekrar Dene', font_size='20sp', halign='center'), size_hint=(0.6, 0.3)).open()
    def dogruyu_goster(self, instance):
        if self.aktif_veri:
            ...
            Popup(title='Doğru Cümle', content=lbl, size_hint=(0.8, 0.4)).open(); SES.oku(self.aktif_veri['cen'])

class Calisma(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.gecmis, self.aktif, self.yon, self.cevrildi = [], None, "tr_to_en", False; self.calisma_listesi = [] 
        layout = BoxLayout(orientation='vertical', padding=20, spacing=15)
        self.kart = OzelButon(text="Başla", background_color=get_color_from_hex('#455A64')); self.kart.font_size = '22sp'; self.kart.bind(on_press=self.cevir)
        self.btn_ses = OzelButon(text="🔊  DİNLE", background_color=(0.4, 0.4, 0.4, 1), size_hint=(1, None), height=90); self.btn_ses.bind(on_press=self.seslendir)
        btns = GridLayout(cols=3, spacing=15, size_hint=(1, None), height=105)
        b1 = OzelButon(text="Geri", background_color=(1,0.6,0,1)); b1.bind(on_press=self.geri)
        b2 = OzelButon(text="Menü", background_color=(0.8,0.2,0.2,1)); b2.bind(on_press=lambda x: setattr(self.manager, 'current', 'menu'))
        b3 = OzelButon(text="İleri", background_color=(0.2,0.8,0.2,1)); b3.bind(on_press=self.ileri)
    ...
    def baslat_ozel(self, mod, liste):
        self.mod = mod; self.calisma_listesi = liste; self.gecmis = []
        if self.calisma_listesi: self.ileri(None)
        else: self.kart.text = "Bu levelda veri yok!"
    def seslendir(self, i):
        if self.aktif:
            if self.mod == "eng_eng": metin = f"{self.aktif['word']}. {self.aktif['ex']}"; SES.oku(metin)
            else: ham_metin = self.aktif['en'] if self.mod=="kelime" else self.aktif['cen']; temiz_metin = re.sub(r"\(.*?\)", "", ham_metin).strip(); SES.oku(temiz_metin)
    def guncelle(self):
        self.kart.markup = True; v = self.aktif
        if not v: return
        if self.mod == "eng_eng":
            if not self.cevrildi: self.kart.ana_renk = get_color_from_hex('#008080'); self.kart.guncelle_canvas(); self.kart.color = (1,1,1,1); self.kart.text = f"[b]{v['word']}[/b]\n\n{v['def']}\n\n[i]\"{v['ex']}\"[/i]"
            else: self.kart.ana_renk = get_color_from_hex('#FBC02D'); self.kart.guncelle_canvas(); self.kart.color = (0,0,0,1); self.kart.text = f"[b]{v['tr']}[/b]"
            return
        if not self.cevrildi:
            self.kart.ana_renk = get_color_from_hex('#37474F'); self.kart.guncelle_canvas(); self.kart.color = (1,1,1,1)
            ...
            self.kart.ana_renk = get_color_from_hex('#FBC02D'); self.kart.guncelle_canvas(); self.kart.color = (0,0,0,1)
            if self.mod == "kelime": self.kart.text = f"[b]{v['en']}[/b]\n[{v['okunus']}]\n---\n{v['tr']}"
            else: self.kart.text = f"[b]{v['cen']}[/b]\n---\n{v['ctr']}"
    ...
    def ileri(self, i): 
        ...
        try:
            ...
            if self.mod == "eng_eng": self.yon = "eng_eng"
            else: self.yon=random.choice(["tr_to_en","en_to_tr"])
...
class AppMain(App):
    def build(self):
        ...
        sm.add_widget(FiilCalismaEkrani(name='fiil_calisma'))
        return sm
